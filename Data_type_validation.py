...
class DbOperationMongoDB:
    ...
    def insertIntoTableGoodData(self,column_name):
        ...
        try:
            database_name="credit_training_log"
            ...
            self.mongodb.drop_collection("training_database","Good_Raw_Data")
            ...
            print(self.good_file_path)
            print("I Love India")
            files = self.az_blob_mgt.get_list_of_blobs_in_container(self.good_file_path)
            print(files,"I Love India")
            ...
            for file in files:
                try:
                    ...
                    df = self.az_blob_mgt.get_csv_file(self.good_file_path,file)
                    print(df)
                    #df.drop("Unnamed: 0", axis=1, inplace=True)
                    #df.drop("Unnamed: 0.1",axis=1,inplace=True)
                    df = df[column_name]
                    df.columns = column_name
                    self.mongodb.insert_df("training_database","Good_Raw_Data",df)
                    ...
                    self.logger_db_writer.log(database_name, collection_name,"File " + file + " was not loaded successfully hence moved tp dir:" + self.bad_file_path)
    ...
    def selectingDatafromtableintocsv(self,):
        ...
        try:
            ...
            df=self.mongodb.get_df_from_collect(training_database,training_collection)
